      ...
      for(j = 0; j < via->lefiVia::numRects(i); j++)
        if(via->lefiVia::rectColorMask(i, j)) {
          fprintf(fout, "    RECT MASK %d ( %f %f ) ( %f %f ) ;\n",
                  via->lefiVia::rectColorMask(i, j), via->lefiVia::xl(i, j),
                  via->lefiVia::yl(i, j), via->lefiVia::xh(i, j),
                  via->lefiVia::yh(i, j));
        }
        else {
...
void lefSpacing(lefiSpacing* spacing) {
  ...
  if(spacing->lefiSpacing::hasStack()) fprintf(fout, "STACK ");
  fprintf(fout, ";\n");
  return;
}
...
void lefViaRuleLayer(lefiViaRuleLayer* vLayer) {
  ...
  if(vLayer->lefiViaRuleLayer::hasRect())
    fprintf(fout, "    RECT  %f %f %f %f ;\n", vLayer->lefiViaRuleLayer::xl(),
            vLayer->lefiViaRuleLayer::yl(), vLayer->lefiViaRuleLayer::xh(),
    ...
    switch(geometry->lefiGeometries::itemType(i)) {
      ...
      case lefiGeomRectIterE:
        ...
        if(rectIter->colorMask != 0) {
          fprintf(fout, "      RECT MASK %d ITERATE ( %f %f ) ( %f %f )\n",
                  rectIter->colorMask, rectIter->xl, rectIter->yl, rectIter->xh,
                  rectIter->yh);
        }
        else {
        ...
        fprintf(fout, "      DO %g BY %g STEP %g %g ;\n", rectIter->xStart,
                rectIter->yStart, rectIter->xStep, rectIter->yStep);
        break;
      ...
      case lefiGeomPolygonIterE:
        ...
        if(polygonIter->colorMask != 0) {
          fprintf(fout, "       POLYGON MASK %d ITERATE ",
        ...
        for(j = 0; j < polygonIter->numPoints; j++)
          fprintf(fout, "       %g %g \n", polygonIter->x[j],
                  polygonIter->y[j]);
        fprintf(fout, "      DO %g BY %g STEP %g %g ;\n", polygonIter->xStart,
                polygonIter->yStart, polygonIter->xStep, polygonIter->yStep);
        break;
      case lefiGeomViaE:
        ...
        if(via->topMaskNum != 0 || via->bottomMaskNum != 0 ||
          ...
          fprintf(fout, "      VIA MASK %d%d%d ( %g %g ) %s ;\n",
                  via->topMaskNum, via->cutMaskNum, via->bottomMaskNum, via->x,
                  via->y, via->name);
        }
        else {
      ...
      case lefiGeomViaIterE:
        ...
        fprintf(fout, "      DO %g BY %g STEP %g %g ;\n", viaIter->xStart,
                viaIter->yStart, viaIter->xStep, viaIter->yStep);
        break;
      default:
...
int arrayBeginCB(lefrCallbackType_e c, const char* name, lefiUserData) {
  ...
  if(status != LEFW_OK) return status;
  return 0;
}
  ...
  if(a->lefiArray::numTrack() > 0) {
    for(i = 0; i < a->lefiArray::numTrack(); i++) {
      ...
      fprintf(fout, "  TRACKS %s, %g DO %d STEP %g\n",
              track->lefiTrackPattern::name(), track->lefiTrackPattern::start(),
...
int fixedMaskCB(lefrCallbackType_e c, int fixedMask, lefiUserData) {
  ...
  if(fixedMask == 1) fprintf(fout, "FIXEDMASK ;\n");
  return 0;
}
...
int layerCB(lefrCallbackType_e c, lefiLayer* layer, lefiUserData) {
  ...
  if(parse65nm) layer->lefiLayer::parse65nmRules();

  // Call parseLef58Type for 5.8 syntax in 5.7
  if(parseLef58Type) layer->lefiLayer::parseLEF58Layer();
  ...
  if(layer->lefiLayer::hasArraySpacing()) {  // 5.7
    ...
    if(layer->lefiLayer::hasLongArray()) fprintf(fout, "LONGARRAY ");
  ...
  if(layer->lefiLayer::hasMinstep()) {
    for(i = 0; i < layer->lefiLayer::numMinstep(); i++) {
      ...
      if(layer->lefiLayer::hasMinstepMinAdjLength(i))
        fprintf(fout, "MINADJLENGTH %g ",
                layer->lefiLayer::minstepMinAdjLength(i));
      if(layer->lefiLayer::hasMinstepMinBetLength(i))
        fprintf(fout, "MINBETLENGTH %g ",
                layer->lefiLayer::minstepMinBetLength(i));
...
int macroCB(lefrCallbackType_e c, lefiMacro* macro, lefiUserData) {
  ...
  if(macro->lefiMacro::isFixedMask()) fprintf(fout, "  FIXEDMASK ;\n");
...
int nonDefaultCB(lefrCallbackType_e c, lefiNonDefault* def, lefiUserData) {
  ...
  if(def->lefiNonDefault::hasHardspacing()) fprintf(fout, "  HARDSPACING ;\n");
    ...
    for(i = 0; i < pin->lefiPin::numForeigns(); i++) {
      if(pin->lefiPin::hasForeignOrient(i))
        fprintf(fout, "    FOREIGN %s STRUCTURE ( %g %g ) %s ;\n",
                ...
                pin->lefiPin::foreignY(i), pin->lefiPin::foreignOrientStr(i));
...
int viaRuleCB(lefrCallbackType_e c, lefiViaRule* viaRule, lefiUserData) {
  ...
  if(viaRule->lefiViaRule::hasGenerate()) fprintf(fout, " GENERATE");
  if(viaRule->lefiViaRule::hasDefault()) fprintf(fout, " DEFAULT");
  fprintf(fout, "\n");
...
void errorCB(const char* msg) { cout << msg << endl; }

void warningCB(const char* msg) { cout << msg << endl; }
...
static void lineNumberCB(int lineNo) {
  // cout << "Parsed " << lineNo << " number of lines!!" << endl;  //lxm:注释了
  return;
}

static void printWarning(const char* str) { fprintf(stderr, "%s\n", str); }

int circuit::ReadLef(const vector< string >& lefStor) {
  FILE* f;
  int retStr = 0;
  int relax = 0;
