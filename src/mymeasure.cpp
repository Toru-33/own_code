...
// lxm:Ctime是CPU时间，是多个核加起来的总时间,Wtime是实际时间
void CMeasure::stop_clock(string task) {
  ...
  welapsed_secs.emplace_back(double(wend - wbegin));

  clock_t cend = clock();
  celapsed_secs.emplace_back(double(cend - cbegin) / CLOCKS_PER_SEC);
}
...
void CMeasure::print_clock() {
  ...
  for(int i = 0; i < tasks.size(); i++) {
    //		int length1 = log10(celapsed_secs[i]) > 0?
    // log10(celapsed_secs[i]): 0; 		int length2 =
    // log10(welapsed_secs[i]) > 0? log10(welapsed_secs[i]): 0;
