...
class MutablePriorityQueue {
 public:
  ...
  explicit MutablePriorityQueue(
      std::function< bool(value_type&, value_type&) > comp)
      : m_funcComp(comp) {
    // the first place is not used
    m_vHandleHeap.emplace_back(-1);
  }
  ...
  handle_type push(const value_type& v) {
    ...
    if(!m_vInvalid.empty()) {
      ...
      m_vElements.emplace_back(v);
      m_vHandleIndex.emplace_back(-1);
    }

    m_vHandleIndex[handle] = m_vHandleHeap.size();
    m_vHandleHeap.emplace_back(handle);
  ...
  value_type pop() {
    ...
    m_vInvalid.emplace_back(m_vHandleHeap[1]);
  ...
  value_type value(handle_type const& handle) { return m_vElements[handle]; }
 ...
 private:
  ...
  std::vector< value_type > m_vElements;     // TO VALUE
  std::vector< handle_type > m_vHandleHeap;  // TO ELEM INDEX
  std::vector< size_t > m_vHandleIndex;      // KEY TO HEAP INDEX
