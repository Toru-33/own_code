...
CircuitParser::CircuitParser(circuit* ckt) : ckt_(ckt) {};

// orient coordinate shift
inline static std::pair< double, double > GetOrientPoint(double x, double y,
                                                         double w, double h,
                                                         int orient) {
  switch(orient) {
    case 0:  // North
...
inline static std::pair< double, double > GetOrientLowerLeftPoint(
    double lx, double ly, double ux, double uy, double w, double h,
    int orient) {
  switch(orient) {
    case 0:                           // North
...
// orient coordinate shift
inline static std::pair< double, double > GetOrientSize(double w, double h,
                                                        int orient) {
  switch(orient) {
...
int CircuitParser::LefLayerCbk(lefrCallbackType_e c, lefiLayer* la,
                               lefiUserData ud) {
...
// SITE parsing
int CircuitParser::LefSiteCbk(lefrCallbackType_e c, lefiSite* si,
                              lefiUserData ud) {
  circuit* ckt = (circuit*)ud;
  ...
  if(si->hasXSymmetry()) {
    mySite->symmetries.emplace_back("X");
  }
  if(si->hasYSymmetry()) {
    mySite->symmetries.emplace_back("Y");
  }
  if(si->has90Symmetry()) {
    mySite->symmetries.emplace_back("R90");
  }
  return 0;
}
...
int CircuitParser::LefStartCbk(lefrCallbackType_e c, const char* name,
                               lefiUserData ud) {
  circuit* ckt = (circuit*)ud;
  switch(c) {
...
int CircuitParser::LefMacroCbk(lefrCallbackType_e c, lefiMacro* ma,
                               lefiUserData ud) {
  circuit* ckt = (circuit*)ud;
  ...
  if(ma->numProperties() > 0) {
    for(int i = 0; i < ma->numProperties(); i++) {
      if(ma->propValue(i)) {
        //        cout << ma->propName(i) << " val: " << ma->propValue(i) <<
        //        endl; printf("%s val: %s\n", ma->propName(i),
        //        ma->propValue(i));
      }
      else {
  ...
  if(ma->hasSiteName()) {
    ...
    topMacro_->sites.emplace_back(ckt->site2id.find(mySite->name)->second);
  }
...
// Set macro's pin
int CircuitParser::LefMacroPinCbk(lefrCallbackType_e c, lefiPin* pi,
                                  lefiUserData ud) {
  circuit* ckt = (circuit*)ud;
  ...
  // lxm:认为有CK/ck/clk/CLK pin的cell就是Flop
  if(pinName == "CK" || pinName == "ck" || pinName == "CLK" ||
     pinName == "clk" || pinName.find("CLK") != string::npos ||
     pinName.find("clk") != string::npos) {
    // cout << "macroName: " << topMacro_->name << " pinName: " << pinName <<
    // endl;
    topMacro_->isFlop = true;
  }
  // cout << "ckt->benchmark: " << ckt->benchmark << endl;
  int a = -1, b = -1;
  if(ckt->benchmark == "mgc_superblue12") {
    a = 60, b = 100;
  }
  else if(ckt->benchmark == "mgc_superblue14" ||
          ckt->benchmark == "mgc_superblue19") {
    a = 50, b = 250;
  }
  else if(ckt->benchmark == "mgc_superblue16_a") {
    a = 10, b = 100;
  }
  else if(ckt->benchmark == "mgc_superblue11_a") {
    a = 100, b = 250;
  }
  // lxm:superblue12:60-100,superblue14/19:50-250,superblue16_a:10-100
  if(topMacro_->name.substr(0, 3) == "MAS") {
    int number = std::stoi(topMacro_->name.substr(3));  // Extract the number
    ...
    for(int j = 0; j < geom->numItems(); j++) {
      switch(geom->itemType(j)) {
        ...
        case lefiGeomRectE:
          ...
          myPin.port.emplace_back(tmpRect);
          break;
...
// Set macro's Obs
int CircuitParser::LefMacroObsCbk(lefrCallbackType_e c, lefiObstruction* obs,
                                  lefiUserData ud) {
  circuit* ckt = (circuit*)ud;
  ...
  for(int i = 0; i < geom->numItems(); i++) {
    ...
    switch(geom->itemType(i)) {
      ...
      case lefiGeomRectE:
        ...
        topMacro_->obses.emplace_back(tmpRect);
        break;
      default:
        break;
    }
...
int CircuitParser::LefEndCbk(lefrCallbackType_e c, const char* name,
                             lefiUserData ud) {
  circuit* ckt = (circuit*)ud;
  switch(c) {
    case lefrMacroEndCbkType:
      //      cout << "Macro: " << topMacro_->name << " is undergoing test" <<
      //      endl;
...
// Row Sort Function
bool SortByRowCoordinate(const opendp::row& lhs, const opendp::row& rhs);
...
int CircuitParser::DefDieAreaCbk(defrCallbackType_e c, defiBox* box,
                                 defiUserData ud) {
  circuit* ckt = (circuit*)ud;
...
int CircuitParser::DefDesignCbk(defrCallbackType_e c, const char* string,
                                defiUserData ud) {
  circuit* ckt = (circuit*)ud;
...
int CircuitParser::DefUnitsCbk(defrCallbackType_e c, double d,
                               defiUserData ud) {
  circuit* ckt = (circuit*)ud;
...
int CircuitParser::DefStartCbk(defrCallbackType_e c, int num, defiUserData ud) {
  circuit* ckt = (circuit*)ud;
  switch(c) {
...
// DEF's End callback
// lxm:更新coreArea和dieArea，更新initial_power信息=>如果最小VDD的高度是rowHeight的偶数倍，则为VDD，否则为VSS
int CircuitParser::DefEndCbk(defrCallbackType_e c, void*, defiUserData ud) {
  circuit* ckt = (circuit*)ud;
  switch(c) {
    case defrSNetEndCbkType:
      // fill initial_power information
      if((int(ckt->minVddCoordiY + 0.5f) / int(ckt->rowHeight + 0.5f)) % 2 ==
         0) {
        cout << "  Initial Power: VDD" << endl;
        ckt->initial_power = VDD;
      }
      else {
        cout << "  Initial Power: VSS" << endl;
        ckt->initial_power = VSS;
      }
      // cout << "ckt->minVddCoordiY: " << ckt->minVddCoordiY << endl;
      break;
    ...
    case defrDesignEndCbkType:
      ...
      if(ckt->prevrows.size() <= 0) {
        cerr << "  ERROR: rowSize is 0. Please define at least one ROW in DEF"
             << endl;
        exit(1);
      }
...
// DEF's ROW parsing
int CircuitParser::DefRowCbk(defrCallbackType_e c, defiRow* ro,
                             defiUserData ud) {
  circuit* ckt = (circuit*)ud;
  ...
  if(ckt->wsite == 0) {
    ckt->wsite =
        int(ckt->sites[myRow->site].width * ckt->DEFdist2Microns + 0.5f);
  }

  ckt->core.xLL = min(1.0 * myRow->origX, ckt->core.xLL);
  ckt->core.yLL = min(1.0 * myRow->origY, ckt->core.yLL);
  ckt->core.xUR =
      max(1.0 * myRow->origX + myRow->numSites * ckt->wsite, ckt->core.xUR);
  ckt->core.yUR = max(1.0 * myRow->origY + ckt->rowHeight, ckt->core.yUR);

  return 0;
}

int CircuitParser::DefPinCbk(defrCallbackType_e c, defiPin* pi,
                             defiUserData ud) {
  circuit* ckt = (circuit*)ud;
...
int CircuitParser::DefComponentCbk(defrCallbackType_e c, defiComponent* co,
                                   defiUserData ud) {
  circuit* ckt = (circuit*)ud;
  cell* myCell = NULL;
  ...
  myCell->width =
      orientSize.first * static_cast< double >(ckt->DEFdist2Microns);
  myCell->height =
      orientSize.second * static_cast< double >(ckt->DEFdist2Microns);
  ...
  // lxm:先给x_coord和y_coord赋上相同值
  myCell->x_coord = myCell->init_x_coord;
  myCell->y_coord = myCell->init_y_coord;
  // fixed cells
  if(myCell->isFixed) {
    // Shift by core.xLL and core.yLL
...
// DEF's COMPONENT parsing
int CircuitParser::DefComponentWriteCbk(defrCallbackType_e c, defiComponent* co,
                                        defiUserData ud) {
  circuit* ckt = (circuit*)ud;
  ...
  // if(co->isFixed() || theCell->is_ff)  // lxm:让FF变为FIXED
  //   fprintf(fout, "+ FIXED ( %d %d ) %s ", placeX, placeY,
  //   orientStr.c_str());
  // if(co->isCover())
  //   fprintf(fout, "+ COVER ( %d %d ) %s ", placeX, placeY,
  //   orientStr.c_str());
  // if(co->isPlaced() && !theCell->is_ff)  // lxm:保证FF不会有placed
  //   fprintf(fout, "+ PLACED ( %d %d ) %s ", placeX, placeY,
  //   orientStr.c_str());

  // lxm:原版
  if(co->isFixed())
    fprintf(fout, "+ FIXED ( %d %d ) %s ", placeX, placeY, orientStr.c_str());
  if(co->isCover())
    fprintf(fout, "+ COVER ( %d %d ) %s ", placeX, placeY, orientStr.c_str());
  if(co->isPlaced())
    fprintf(fout, "+ PLACED ( %d %d ) %s ", placeX, placeY, orientStr.c_str());

  if(co->isUnplaced()) {
    fprintf(fout, "+ UNPLACED ");
    if((placeX != -1) || (placeY != -1)) {
      fprintf(fout, "( %d %d ) %s ", placeX, placeY, orientStr.c_str());
    }
  }
...
// DEF's NET
// lxm:修改后的NET读取
int CircuitParser::DefNetCbk(defrCallbackType_e c, defiNet* dnet,
                             defiUserData ud) {
  circuit* ckt = (circuit*)ud;
  net* myNet = NULL;
  ...
  for(int i = 0; i < dnet->numConnections(); i++) {
    ...
    if(strcmp(dnet->instance(i), "PIN") != 0) {
      ...
      // lxm:为FF最开始的移动做准备
      myNet->xLL = min(myNet->xLL, ckt->cells[myPin->owner].init_x_coord +
                                       myPin->x_offset * ckt->DEFdist2Microns);
      myNet->yLL = min(myNet->yLL, ckt->cells[myPin->owner].init_y_coord +
                                       myPin->y_offset * ckt->DEFdist2Microns);
      myNet->xUR = max(myNet->xUR, ckt->cells[myPin->owner].init_x_coord +
                                       myPin->x_offset * ckt->DEFdist2Microns);
      myNet->yUR = max(myNet->yUR, ckt->cells[myPin->owner].init_y_coord +
                                       myPin->y_offset * ckt->DEFdist2Microns);
      cell* theCell = &ckt->cells[myPin->owner];
      theCell->connected_nets.emplace_back(myNetId);
      theCell->pins.emplace_back(myPin->id);
    }
    else {
      myNet->is_IONET = true;
    }
...
int CircuitParser::DefSNetCbk(defrCallbackType_e c, defiNet* swire,
                              defiUserData ud) {
  circuit* ckt = (circuit*)ud;

  // Check the VDD values
  if(strcmp("vdd", swire->name()) == 0 || strcmp("VDD", swire->name())) {
    if(swire->numWires()) {
      for(int i = 0; i < swire->numWires(); i++) {
        ...
        for(int j = 0; j < wire->numPaths(); j++) {
          ...
          if(isMeetFirstLayer && !isViaRelated) {
            ckt->minVddCoordiY =
                (ckt->minVddCoordiY < y1) ? ckt->minVddCoordiY : y1;
            //            cout << x1 << " " << y1 << " " << x3 << " " << y3 <<
            //            endl;
          }
        }
...
int CircuitParser::DefRegionCbk(defrCallbackType_e c, defiRegion* re,
                                defiUserData ud) {
  circuit* ckt = (circuit*)ud;
  ...
  for(int i = 0; i < re->numRectangles(); i++) {
    ...
    // push rect info
    curGroup->regions.emplace_back(tmpRect);
  }
...
int CircuitParser::DefGroupNameCbk(defrCallbackType_e c, const char* name,
                                   defiUserData ud) {
  circuit* ckt = (circuit*)ud;
...
int CircuitParser::DefGroupMemberCbk(defrCallbackType_e c, const char* name,
                                     defiUserData ud) {
  circuit* ckt = (circuit*)ud;
  ...
  for(auto& curCell : ckt->cells) {
    ...
    if(strncmp(topGroup_->tag.c_str(), curCell.name.c_str(),
      ...
      topGroup_->siblings.emplace_back(&curCell);
      ckt->group_cells.emplace_back(&curCell);  // lxm:记录group中所有cell
