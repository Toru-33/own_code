...
int defrData::sublex(YYSTYPE *pYylval)
{
   ...
   for (;;)
   {
      if (!DefGetToken(&deftoken, &deftokenLength))
      { /* get a raw token */
         return 0;
      }
      fc = deftoken[0];
      ...
      if (defGetKeyword(uc_token, &result))
      {
         ...
         { /* history - get up to ';' */
            ...
            while (1)
            {
               ...
               prev = c;
               History_text.emplace_back(c);
            }
            History_text.emplace_back('\0');
         }
...
void defrData::defInfo(int msgNum, const char *s)
{
   ...
   for (i = 0; i < settings->nDDMsgs; i++)
   { /* check if info has been disable */
...
void defrData::defWarning(int msgNum, const char *s)
{
   ...
   for (i = 0; i < settings->nDDMsgs; i++)
   { /* check if warning has been disable */
      if (settings->disableDMsgs[i] == msgNum)
