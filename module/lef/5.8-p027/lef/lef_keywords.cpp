...
void lefReloadBuffer()
{
    ...
    if (lefData->encrypted)
    {
        ...
        { // an encrypted file, but user does not allow to read one
...
int lefGetc()
{
    if (lefData->input_level >= 0)
    { // Token has been getting from
        const char *ch, *s;
...
GetToken(char **buffer, int *bufferSize)
{
        ...
        do
        {
            ...
            */
            if (!lefSettings->DisPropStrProcess)
            {
                ...
                if (ch == '\\')
                { // got a \, save the lefData->next char only
                    ch = lefGetc();
    ...
    else if (lefSettings->ShiftCase)
    { // we are case insensitive, use a different loop
        for (;; ch = lefGetc())
        {
            /* 5/6/2008 - CCR 556818
            ** Check if the ch is a valid ascii character 0 =< ch < 128
...
int lefsublex()
{
    ...
    for (;;)
    {
        ...
        {
            for (fc = lefGetc();; fc = lefGetc())
            { // so skip to the end of line
                if (fc == EOF)
                    return 0;
                if (fc == '\n')
                {
    ...
    if (isdigit(fc) || fc == '.' || (fc == '-' && lefData->current_token[1] != '\0'))
    {
        ...
        if (lefData->lefNoNum < 0 && *ch == '\0')
        { // did we use the whole string?
            return NUMBER;
        }
        else
        ...
        if (lefGetKeyword(lefData->uc_token, &result))
        {
            ...
            { // history - get up to ';'
                ...
                for (;;)
                {
                    ...
                    prev = c;
                    lefData->Hist_text.emplace_back(c);
                }
                lefData->Hist_text.emplace_back('\0');
            }
            else if (K_BEGINEXT == result)
            { // extension, get up to end
                int cc;
                int foundTag = 0;
                int notEmpTag = 0;
                int begQuote = 0;
                ...
                for (cc = lefGetc();; cc = lefGetc())
                {
                    ...
                    else
                    {
                        ...
                        lefData->Hist_text.emplace_back(cc);
                        if (cc != ' ')
                        {
                            if (cc == '\"')
                            { // found a quote
    ...
    { // it should be a punctuation character
        if (lefData->current_token[1] != '\0')
        {
            ...
            else if (lefData->current_token[0] == '_')
            { // name starts with _, return as T_STRING
                yylval.string = ringCopy(lefData->current_token);
                return T_STRING;
            }
            else
        ...
        else if (strlen(lefData->current_token) > 2 || strlen(lefData->current_token) == 0)
        {
            lefError(6016, "Odd punctuation found.");
            lefData->hasFatalError = 1;
            return 0;
             ...
             const char *s)
{
    ...
    if (lefSettings->MsgLimit[msgNum] > 0)
    {
        if (lefData->msgLimit[0][msgNum] >= lefSettings->MsgLimit[msgNum])
        { // over the limit
            char msgStr[100];
